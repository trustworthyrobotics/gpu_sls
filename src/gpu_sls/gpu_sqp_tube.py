...
def compute_search_direction(
    ...
    def run_tube_mpc(_):
        E = disturbance(X)
        K_fixed = controller_pas(Q, R, M, A, B)
        Jh = jnp.zeros((T + 1, nc, sls_config.gradient_window, nx))
        (
            dX, dU, dV, w1, y1, rho1, rho_grad1,
            _, tube_backoffs, muN, a1, b1, converged_admm,
        ) = tube_mpc(
            admm_config, Q, q, R, r, M, A, B, c,
            C_all, D_all, f_all, w, y, rho, rho_grad,
            Jh, a, b, sls_config.gradient_window, K_fixed, E,
            slack_weight,
        )

        # Obstacle rows are appended only inside the QP. Keep the carried
        # tightening shape identical to gpu_sqp's nonlinear-constraint API.
        backoffs = tube_backoffs[:, : nc - n_obs]
        Phi_x = jnp.zeros_like(Phi_x_ws)
        Phi_u = jnp.zeros_like(Phi_u_ws)
        betaN = jnp.ones_like(beta_ws) * jnp.asarray(1e-10, beta_ws.dtype)
        Phi_x_I = jnp.zeros_like(Phi_x_I_ws)
        Phi_u_I = jnp.zeros_like(Phi_u_I_ws)
        return (
            dX, dU, dV, w1, y1, rho1, rho_grad1, backoffs,
            Phi_x, Phi_u, betaN, muN, Phi_x_I, Phi_u_I,
            a1, b1, converged_admm,
        )
    ...
    dX, dU, dV, w1, y1, rho1, rho_grad1, backoffs, Phi_x, Phi_u, betaN, muN, Phi_x_I, Phi_u_I, a1, b1, converged_admm = lax.cond(
        use_nominal, run_nominal, run_tube_mpc, operand=None
    )
