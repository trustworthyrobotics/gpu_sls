...
@partial(jax.jit, static_argnums=(0, 1, 2, 3))
def reference_barrel_roll_fixed_time(N, dt, n_joints, n_contact, foot0, q0):
    ...
    moving_steps = n1 + n2 + n3 + n4
    lateral_speed = -0.14 / (moving_steps * dt)

    def base_block(start, count, height, vertical_speed):
        indices = jnp.arange(start, start + count)
        p = jnp.zeros((count, 3))
        p = p.at[:, 1].set(lateral_speed * indices * dt)
        p = p.at[:, 2].set(height)
        dp = jnp.tile(jnp.array([0.0, lateral_speed, 0.0]), (count, 1))
        dp = dp.at[:, 2].set(vertical_speed)
        return p, dp

    p1, dp1 = base_block(0, n1, base_height, 0.0)
    ...
    p5, dp5 = base_block(moving_steps, n5, landing_height, 0.0)
    p6, dp6 = base_block(moving_steps + n5, n6, landing_height, 0.0)
    landing_y = p4[-1, 1]
    p5 = p5.at[:, 1].set(landing_y)
    p6 = p6.at[:, 1].set(landing_y)
    dp5 = dp5.at[:, 1].set(0.0)
    dp6 = dp6.at[:, 1].set(0.0)
