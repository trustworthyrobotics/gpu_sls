...
Q_POS = np.array([15.0, 15.0, 15.0])
Q_ANG = np.array([5.0, 5.0, 5.0])
Q_VEL = np.array([0.01, 0.01, 0.01])
Q_RATE = np.array([0.5, 0.5, 0.5])
...
def disturbance_spatial_scale(
    px,
    py,
    params=DISTURBANCE_FIELD,
):
    x_window = (
        0.5
        * (1.0 + np.tanh(params.kx * (px - params.x_min)))
        * 0.5
        * (1.0 - np.tanh(params.kx * (px - params.x_max)))
    )

    y_window = (
        0.5
        * (1.0 + np.tanh(params.ky * (py - params.y_min)))
        * 0.5
        * (1.0 - np.tanh(params.ky * (py - params.y_max)))
    )
...
def quadrotor_xdot_np(x, u):
    ...
    R = rotation_matrix_np(
        phi,
        theta,
        psi,
    )

    E = euler_angle_rates_matrix_np(
        phi,
        theta,
    )
...
def build_gate_bar_geometry_np(plan):
    ...
    if not all(
        ...
        raise ValueError(
            "Physical gate geometry is required for the "
            "gate-directed adversarial disturbance."
        )
...
def nearest_gate_bar_direction_np(
    pos,
    gate_bar_geometry,
    eps=1e-12,
):
    ...
    centers = gate_bar_geometry[
        "centers"
    ]

    half_sizes = gate_bar_geometry[
        "half_sizes"
    ]

    axis_u = gate_bar_geometry[
        "axis_u"
    ]

    axis_v = gate_bar_geometry[
        "axis_v"
    ]

    normals = gate_bar_geometry[
        "normals"
    ]
...
def plant_step(
    ...
    if (
        gate_centers.ndim != 2
        or gate_centers.shape[1] != 3
    ):
        raise ValueError(
            "gate_bar_geometry['gate_centers'] "
            "must have shape (num_gates, 3)."
        )
...
# ============================================================
# JAX online tracking model
# ============================================================

def disturbance_spatial_scale_jax(
    px,
    py,
    params=DISTURBANCE_FIELD,
):
...
def euler_angle_rates_matrix_jax(
    phi,
    theta,
):
    theta_safe = jnp.clip(
...
def quadrotor_xdot_jax(
    ...
    R = rotation_matrix_jax(
        phi,
        theta,
        psi,
    )

    E = euler_angle_rates_matrix_jax(
        phi,
        theta,
    )
...
def make_tracking_cost(horizon):
    ...
    def cost(
        ...
        *,
        ...
        uref_idx = jnp.minimum(
            t,
            uref_nodes.shape[0] - 1,
        )
...
def make_tracking_gate_frame_constraints(
    ...
    gate_centers = jnp.asarray(
        gate_centers,
        dtype=DTYPE,
    )

    gate_normals = jnp.asarray(
        gate_normals,
        dtype=DTYPE,
    )

    gate_axis_u = jnp.asarray(
        gate_axis_u,
        dtype=DTYPE,
    )

    gate_axis_v = jnp.asarray(
        gate_axis_v,
        dtype=DTYPE,
    )

    opening_half_widths = jnp.asarray(
        opening_half_widths,
        dtype=DTYPE,
    )
...
def build_gpu_sls_mpc(
    ...
    *,
    ...
    if solver_mode == "initial_full":
        ...
        raise ValueError(
            f"Unknown solver_mode={solver_mode!r}; "
            "expected 'initial_full' or 'rti'."
        )
    ...
    disturbance = make_tracking_disturbance(
        TRACK_NX,
        disturbance_mag,
    )
...
def load_offline_plan(filename):
    ...
    with np.load(
        ...
        waypoint_centers = optional_array(
            "waypoint_centers"
        )

        waypoint_half_widths = optional_array(
            "waypoint_half_widths"
        )

        waypoint_axis_u = optional_array(
            "waypoint_axis_u"
        )

        waypoint_axis_v = optional_array(
            "waypoint_axis_v"
        )

        waypoint_normals = optional_array(
            "waypoint_normals"
        )

        gate_centers = optional_array(
            "gate_centers"
        )

        gate_normals = optional_array(
            "gate_normals"
        )

        gate_axis_u = optional_array(
            "gate_axis_u"
        )

        gate_axis_v = optional_array(
            "gate_axis_v"
        )

        gate_opening_half_widths = optional_array(
            "gate_opening_half_widths"
        )
    ...
    if X_full.shape[1] < NX:
        raise ValueError(
            f"Expected at least {NX} state columns, "
    ...
    if len(phase_times) != len(
        segment_lengths
    ):
        raise ValueError(
            "phase_times and waypoint_steps imply "
...
def compute_tracking_objective_np(
    X_pred,
    U_pred,
    X_ref,
    U_ref,
...
def solve_mpc_step(
    ...
    X_aug_ref, parameter = (
        build_augmented_local_reference(
            X_ref_np=X_ref_np,
            U_ref_np=U_ref_np,
            dt_ref_np=dt_ref_np,
    ...
    X_pred = jax.block_until_ready(
        X_pred
    )

    U_pred = jax.block_until_ready(
        U_pred
    )
    ...
    if not np.all(
        ...
        raise RuntimeError(
            "SLS response Phi_x contains NaN or Inf."
        )
    ...
    print(
        "Terminal SLS tube: "
        f"xyz_halfwidth=["
        f"{terminal_xyz_halfwidth[0]:.6f}, "
        f"{terminal_xyz_halfwidth[1]:.6f}, "
        f"{terminal_xyz_halfwidth[2]:.6f}] m  "
        f"xyz_radius_bound="
        f"{terminal_xyz_radius_bound:.6f} m"
    )
    ...
    if not np.all(
        ...
        raise RuntimeError(
            "MPC produced NaN or Inf in its predicted state trajectory."
        )
    ...
    if not np.all(
        ...
        raise RuntimeError(
            "MPC produced NaN or Inf in its predicted control trajectory."
        )
    ...
    if (
        ...
        raise RuntimeError(
            ...
            f"u0={u0_np}, "
            f"bounds=[{U_MIN}, {U_MAX}]."
        )

    objective = (
        compute_tracking_objective_np(
            X_pred_np,
            U_pred_np,
            X_ref_np,
            U_ref_np,
    ...
    if not np.isfinite(
        ...
        raise RuntimeError(
            "MPC produced a nonfinite tracking objective."
        )
...
def shift_warm_start(
    ...
    X_guess = np.empty_like(
        X_sol
    )

    U_guess = np.empty_like(
        U_sol
    )
...
def run_tracking(
    ...
    if len(
        ...
        raise ValueError(
            "Expected exactly two waypoint steps "
            "(pre/post) per physical gate."
        )
    ...
    print(
        f"Offline GPU-SLS plan: "
        f"{plan['filename']}"
    )

    print(
        f"Offline plan intervals: "
        f"{N}"
    )
    ...
    print(
        f"Progress search window: "
        f"{progress_search_window} nodes"
    )
    ...
    print(
        f"Max controller steps: "
        f"{max_control_steps}"
    )

    print(
        "Plant adversary: XY position disturbance "
        "toward closest gate bar (z delta = 0)"
    )

    print(
        f"E_MAG_MODEL: "
        f"{E_MAG_MODEL:.6f}  "
        "(delta_p = dt * E_MAG_MODEL * "
        "[w_x, w_y, 0], ||w_xy||_2 <= 1)"
    )

    print(
        "Robust-model state-dependent vy magnitude: "
        f"{disturbance_mag}"
    )

    print(
        "Gate-bar collision detection: DISABLED"
    )
    ...
    print(
        f"Subsequent MPC solves: RTI, "
        f"{GPU_SLS_RTI_SQP_ITERS} SQP iterations per step"
    )
    ...
    for control_step in range(
        ...
        if (
            progress_idx >= N
            and final_pos_error_before
            <= goal_tol
        ):
            reached_goal = True
            break

        mpc_progress_idx = min(
            progress_idx,
            N - 1,
        )
        ...
        try:
            ...
            if control_step == 0:
                ...
                print(
                    f"Initial full solve completed in "
                    f"{1000.0 * solve_time:.3f} ms. "
    ...
    # ========================================================
    # Minimum-distance-to-final-goal rollout duration
    #
    # X_closed_loop[k] is the state after k plant intervals.
    # Therefore its physical rollout time is
    #
    #     t_k = sum(dt_applied_history[:k]).
    #
    # X_closed_loop[0] corresponds to t = 0.
    # ========================================================
    ...
    final_goal_distance_history = np.linalg.norm(
        X_closed_loop[:, :3]
        - final_goal_position[
            None,
            :
    ...
    min_goal_distance_idx = int(
        np.argmin(
            final_goal_distance_history
        )
    )

    min_goal_distance = float(
        final_goal_distance_history[
            min_goal_distance_idx
        ]
    )

    rollout_duration_at_min_goal_distance = float(
        np.sum(
            dt_applied_history[
                :min_goal_distance_idx
            ]
    ...
    print(
        f"Reached final goal: "
        f"{reached_goal}"
    )

    print(
        f"Rollout duration [s]: "
        f"{rollout_duration:.6f}"
    )

    print(
        f"Minimum final-goal distance [m]: "
        f"{min_goal_distance:.6f}"
    )

    print(
        "Closest final-goal approach at rollout step: "
        f"{min_goal_distance_idx}"
    )

    print(
        "Rollout duration at minimum final-goal distance [s]: "
        f"{rollout_duration_at_min_goal_distance:.6f}"
    )

    print(
        f"Controller steps executed: "
        f"{num_control_steps}"
    )
    ...
    if len(
        ...
        print(
            "Only the initialization solve was completed; "
            "no RTI timing samples yet."
        )
    ...
    return {
        ...
        # New final-goal metrics.
        "final_goal_distance_history": final_goal_distance_history,
        "min_goal_distance": min_goal_distance,
        "min_goal_distance_idx": min_goal_distance_idx,
        "rollout_duration_at_min_goal_distance": (
            rollout_duration_at_min_goal_distance
        ),
...
def plot_tracking_top_down(
    ...
    if (
        ...
        if (
            ...
            and waypoint_axis_u.shape[0]
            == waypoint_centers.shape[0]
            and waypoint_normals.shape[0]
            == waypoint_centers.shape[0]
        ):
            for (
                center,
    ...
    print(
        f"Saved top-down tracking plot to: "
        f"{save_path}"
    )
...
def plot_tracking_error(
    ...
    print(
        f"Saved tracking-error plot to: "
        f"{save_path}"
    )
...
def parse_args():
    ...
    parser.add_argument(
        "input",
        nargs="?",
        default="multiphase_trajectory_dist_nom.npz",
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            "Magnitude of the state-dependent vy uncertainty "
            "channel kept inside the robust MPC model. "
    ...
    parser.add_argument(
        ...
        help=(
            "Number of offline trajectory nodes ahead of "
            "the current progress index searched for the "
            "nearest point."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Final XYZ position tolerance required to stop "
            "after reaching the final progress node."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default is 2 times the number of offline intervals."
        ),
    )
...
def main():
    ...
    np.savez(
        ...
        X_closed_loop=result[
            "X_closed_loop"
        ],

        U_closed_loop=result[
            "U_closed_loop"
        ],
        ...
        X_mpc_nominal_history=result[
            "X_mpc_nominal_history"
        ],

        U_mpc_nominal_history=result[
            "U_mpc_nominal_history"
        ],

        tube_xyz_halfwidth_history=result[
            "tube_xyz_halfwidth_history"
        ],

        mpc_horizon=np.array(
            args.horizon
        ),
        ...
        # ====================================================
        # New closest-final-goal metrics
        # ====================================================

        final_goal_distance_history=result[
            "final_goal_distance_history"
        ],
        ...
        min_goal_distance_idx=np.array(
            result[
                "min_goal_distance_idx"
            ]
        ),

        rollout_duration_at_min_goal_distance=np.array(
            result[
                "rollout_duration_at_min_goal_distance"
            ]
        ),
    ...
    print(
        f"Saved closed-loop tracking result to: "
        f"{output_path}"
    )
    ...
    plot_tracking_top_down(
        ...
        waypoint_centers=plan[
            "waypoint_centers"
        ],
        waypoint_half_widths=plan[
            "waypoint_half_widths"
        ],
        waypoint_axis_u=plan[
            "waypoint_axis_u"
        ],
        waypoint_normals=plan[
            "waypoint_normals"
        ],
        gate_centers=plan[
            "gate_centers"
        ...
        gate_opening_half_widths=plan[
            "gate_opening_half_widths"
        ],
        gate_bar_thickness=plan[
            "gate_bar_thickness"
        ],
        gate_depth=plan[
            "gate_depth"
