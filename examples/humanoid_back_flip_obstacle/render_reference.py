"""Render the nominal H1 corridor-backflip reference without simulating it."""
...
DEFAULT_OUTPUT = SCRIPT_DIR / "humanoid_backflip_obstacle_reference.mp4"
...
def main():
    ...
    frame_count = render_video(
        ...
        obstacle_center=np.asarray(problem.OBSTACLE_CENTERS, dtype=np.float64),
        obstacle_size=np.asarray(problem.OBSTACLE_SIZES, dtype=np.float64),
    )
    print("Rendered the nominal obstacle reference by direct state assignment.")
    print(
        "Corridor obstacle centers/sizes:\n"
        f"{np.asarray(problem.OBSTACLE_CENTERS)}\n"
        f"{np.asarray(problem.OBSTACLE_SIZES)} m"
    )
