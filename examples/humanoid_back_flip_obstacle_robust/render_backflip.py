"""Render an optimized H1 corridor backflip to a slow-motion MP4.

Slow motion changes playback timing only; the saved optimized states and
minimum-time phase durations are not modified.

Examples:
    python render_backflip.py
    python render_backflip.py humanoid_backflip_min_time.npz --slowmo 6
...
def load_trajectory(filename: Path):
    ...
    return states, node_times, phase_times, obstacle_centers, obstacle_sizes
...
def main():
    ...
    states, node_times, phase_times, obstacle_centers, obstacle_sizes = (
