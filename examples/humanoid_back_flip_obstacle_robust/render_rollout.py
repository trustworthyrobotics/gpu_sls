"""Render the zero-disturbance nonlinear rollout of an optimized H1 backflip.

This expects the optimization result NPZ to contain:
    X_zero_disturbance_rollout

The rollout is rendered using the SAME optimized node times / phase durations.
Slow motion changes playback timing only.

Examples:
    python render_backflip_rollout.py
    python render_backflip_rollout.py humanoid_backflip_obstacle_min_time.npz
    python render_backflip_rollout.py --slowmo 1
    python render_backflip_rollout.py --slowmo 4 --output backflip_rollout_slowmo.mp4
"""
...
def load_rollout(filename: Path):
    ...
    with np.load(filename, allow_pickle=False) as result:
        rollout_key = "X_zero_disturbance_rollout"

        if rollout_key not in result:
            raise KeyError(
                f"{filename} does not contain '{rollout_key}'.\n"
                "Run the optimization version that saves the sequential "
                "zero-disturbance dynamics rollout first."
            )

        if "phase_times" not in result:
            raise ValueError("The NPZ must contain phase_times.")
        ...
        if "node_times" in result:
            node_times = np.asarray(
                result["node_times"], dtype=np.float64
            )
    ...
    if states.ndim != 2:
        raise ValueError(
            f"Rollout states must be 2-D, got shape {states.shape}."
        )

    if node_times.size != states.shape[0]:
        raise ValueError(
            "X_zero_disturbance_rollout and node_times have incompatible "
            f"shapes: {states.shape} vs {node_times.shape}."
        )

    if not np.all(np.isfinite(states)):
        raise ValueError("Rollout states contain non-finite values.")

    if not np.all(np.isfinite(node_times)):
        raise ValueError("node_times contain non-finite values.")
...
def main():
    ...
    parser.add_argument(
        ...
        default=4.0,
        help=(
            "Playback slowdown factor; 4 renders the rollout "
            "four times slower."
        ),
    )
    ...
    if output is None:
        output = trajectory_path.with_name(
            f"{trajectory_path.stem}_zero_rollout.mp4"
        )
    ...
    print("Rendered state source: X_zero_disturbance_rollout")
    print(f"Optimized phase times: {phase_times}")
    print(f"Physical duration: {node_times[-1]:.6f} s")
    print(f"Slow motion: {args.slowmo:g}x")
    print(
        f"Playback duration: {playback_node_times[-1]:.6f} s"
    )
