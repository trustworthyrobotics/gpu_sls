...
def solve_barrel_roll(mpc, data, x0, reference, parameter):
    """Solve feasibility first, then warm-start the minimum-time problem."""
    ...
    feasibility_W = dict(data.W)
    feasibility_W["time"] = jnp.zeros_like(data.W["time"])
    feasibility_result = run_solver(
        feasibility_W, X_guess, U_guess, initial_workspace
    )

    # Reuse both the primal trajectory and every ADMM/SLS workspace.  This is
    # the critical continuation step: time minimization starts only after the
    # nonlinear motion has had a chance to become dynamically feasible.
    minimum_time_result = run_solver(
        data.W,
        feasibility_result[0],
        feasibility_result[1],
        feasibility_result[2:],
    )
    return feasibility_result, minimum_time_result
...
def main(*, dry_run_only=False, output_dir=DIR_PATH):
    ...
    start = timer()
    feasibility_result, result = solve_barrel_roll(
        mpc, data, x0, reference, parameter
    )
    X_feasibility, U_feasibility = feasibility_result[:2]
    X, U = result[:2]
    converged_admm = result[-1]
    X.block_until_ready()
    solve_time = timer() - start

    feasibility_violation, _, _ = evaluate_constraint_violation(
        constraints, X_feasibility, U_feasibility
    )
    feasibility_dynamics_defect, _, _ = evaluate_dynamics_defect(
        mpc.dynamics, X_feasibility, U_feasibility, parameter
    )
    ...
    print(
        "Feasibility-stage violation/dynamics defect: "
        f"{feasibility_violation:.6e} / {feasibility_dynamics_defect:.6e}"
    )
    ...
    np.savez(
        ...
        feasibility_stage_constraint_violation=np.asarray(feasibility_violation),
        feasibility_stage_dynamics_defect=np.asarray(feasibility_dynamics_defect),
