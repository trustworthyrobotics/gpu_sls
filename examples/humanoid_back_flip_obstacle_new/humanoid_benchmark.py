...
def main(*, dry_run_only=False, output_dir=DIR_PATH, benchmark_runs=BENCHMARK_RUNS):
    configure_problem()
    ...
    # ------------------------------------------------------------------
    # Benchmark: independent solves from identical initialization.
    # ------------------------------------------------------------------
    # Do not use MPCWrapper.reset() here. Different wrapper revisions expose
    # different reset signatures, while this direct OCP calls _solve with x0,
    # X_guess, and U_guess explicitly. Rebuilding MPCData is therefore the
    # simplest branch-independent way to restore a pristine SQP/SLS/ADMM
    # workspace before every independent solve.

    # Warm up JIT compilation before collecting benchmark timings.
    print("Compiling / warming up solver...")
    data = mpc.make_data()
    warmup_result = solve_backflip(
        mpc, data, x0, reference, parameter
    )
    ...
    for run_idx in range(benchmark_runs):
        # Recreate all solver workspace before each solve so every timed run
        # starts from identical primal/dual/penalty/SLS initialization. This is
        # intentionally outside the timed region, matching the prior benchmark.
        data = mpc.make_data()

        start = timer()
        result = solve_backflip(
            mpc, data, x0, reference, parameter
        )
        X_run, U_run = result[:2]

        # JAX dispatch is asynchronous. Synchronize before stopping the timer
        # so the measurement reflects actual wall-clock solve time.
        X_run.block_until_ready()
        solve_time_run = timer() - start

        if (
            not bool(jnp.all(jnp.isfinite(X_run)))
            or not bool(jnp.all(jnp.isfinite(U_run)))
        ):
            raise FloatingPointError(
                f"Backflip benchmark run {run_idx + 1} returned a non-finite "
                "trajectory; result not saved."
            )

        # H1 stores unconstrained duration logits in X. Decode them before
        # reporting the physical maneuver duration.
        phase_times_run = np.asarray(phase_durations(X_run[0]))
        optimal_time_run = float(np.sum(phase_times_run))
        solve_times.append(solve_time_run)
        optimal_times.append(optimal_time_run)

        print(
            f"Run {run_idx + 1:02d}/{benchmark_runs}: "
            f"solve={solve_time_run:.6f} s, "
            f"optimal_time={optimal_time_run:.6f} s"
        )
        last_result = result
    ...
    mean_optimal_time = float(np.mean(optimal_times))
    std_optimal_time = float(np.std(optimal_times))

    print()
    print("=" * 60)
    print(f"Results over {benchmark_runs} independent timed solves")
    print("=" * 60)
    print(f"Solve time:   {mean_solve_time:.6f} ± {std_solve_time:.6f} s")
    print(f"Optimal time: {mean_optimal_time:.6f} ± {std_optimal_time:.6f} s")
    print()
    ...
    # Continue diagnostics and saving with the final timed run.
    result = last_result
    X, U = result[:2]
    converged_admm = result[-1]
    solve_time = float(solve_times[-1])
    ...
    np.savez(
        ...
        benchmark_runs=np.asarray(benchmark_runs),
        benchmark_solve_times=solve_times,
        benchmark_optimal_times=optimal_times,
        benchmark_mean_solve_time=np.asarray(mean_solve_time),
        benchmark_std_solve_time=np.asarray(std_solve_time),
        benchmark_mean_optimal_time=np.asarray(mean_optimal_time),
        benchmark_std_optimal_time=np.asarray(std_optimal_time),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--benchmark-runs", type=int, default=BENCHMARK_RUNS,
        help="Number of timed solves after one untimed JIT warm-up (default: 20).",
    )
